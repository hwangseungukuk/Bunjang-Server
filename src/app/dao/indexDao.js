...
async function mainFeed(userIndex) {
  const connection = await pool.getConnection(async (conn) => conn);
  const mainFeedQuery = `
  SELECT p.postIndex, p.productName, p.price, pi.postImgURL, u.userIndex, u.userName, u.profileImgURL,
  (CASE
  ...
  (SELECT COUNT(*) FROM Jjim j WHERE j.postIndex = p.postIndex AND j.userIndex = ?) AS didJjim
  FROM Post p LEFT JOIN User u ON p.userIndex = u.userIndex
  LEFT JOIN postImg pi ON pi.postIndex = p.postIndex AND pi.isFirst = 1
  WHERE (p.onlyMyPlace = 0 OR (p.onlyMyPlace = 1 AND p.place = (SELECT u1.place FROM User u1 WHERE u1.userIndex = ?)));
  `;

  var params = [userIndex, userIndex];
...
// 게시글 찜하기
async function doJjim(userIndex, postIndex) {

  const connection = await pool.getConnection(async (conn) => conn);
  const checkJjimQuery = `
  SELECT COUNT(*) AS didJjim FROM Jjim j
  WHERE j.postIndex = ? AND j.userIndex = ?;
  `
  const doJjimQuery = `
  ...
  const [checkJjimRows] = await connection.query(
    checkJjimQuery,
    jjimParams
  );

  const didJjim = JSON.parse(JSON.stringify(checkJjimRows))[0].didJjim;
  ...
  const [checkJjimRows2] = await connection.query(
    checkJjimQuery,
    jjimParams
  );

  const didJjim2 = JSON.parse(JSON.stringify(checkJjimRows2))[0].didJjim;
